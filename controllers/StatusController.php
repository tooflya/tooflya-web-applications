...
class StatusController extends BaseController
{
  ...
  public function getServerBaseInfo()
  {
    ...
    $this->templates->assign_array("SELECT * FROM (SELECT *, COUNT(*) AS `count`, DAY(`time`) AS `day` FROM `uptime` WHERE `time` > (NOW( ) - INTERVAL 7 DAY) GROUP by `day` ORDER by `time` DESC LIMIT 7) AS `table` GROUP BY `table`.`day` ORDER by `table`.`time` ASC", 'uptime');
    $this->templates->assign_array("SELECT AVG(`speed`) AS `speed`, AVG(`ping`) AS `ping`, DAY(`time`) AS `day`, `time` FROM (SELECT * FROM `bandwidth` WHERE `time` > (NOW( ) - INTERVAL 7 DAY) GROUP by DAY(`time`) ORDER by `time` DESC LIMIT 7) AS `table` GROUP BY `day` ORDER by `table`.`time` ASC", 'bandwidth');
    $this->templates->assign_array("SELECT AVG(`one`) AS `one`, AVG(`five`) AS `five`, AVG(`fifteen`) AS `fifteen`, DAY(`time`) AS `day`, `time` FROM (SELECT * FROM `la` WHERE `time` > (NOW( ) - INTERVAL 7 DAY) GROUP by DAY(`time`) ORDER by `time` DESC LIMIT 7) AS `table` GROUP BY `day` ORDER by `table`.`time` ASC", 'la');
